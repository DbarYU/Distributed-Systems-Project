...
public class LeaderElection {
    ...
    public LeaderElection(PeerServer server, LinkedBlockingQueue<Message> incomingMessages, Set<Long> failedPeers) {
    ...
    public synchronized Vote lookForLeader() {
        try {
            long id = this.peerServer.getPeerState() == PeerServer.ServerState.OBSERVER ? Long.MIN_VALUE : this.peerServer.getServerId();
            currVote = new Vote(id,this.peerServer.getPeerEpoch());
            ...
            while(true){
                ...
                if (msg == null){
                    ...
                    if(noti.getState() == PeerServer.ServerState.OBSERVER)
                        isValid = false;
                    if(failedPeers.contains(noti.getProposedLeaderID()) || failedPeers.contains(noti.getSenderID()))
                        isValid = false;
                    ...
                    if(isValid && supersedesCurrentVote(noti.getProposedLeaderID(),noti.getPeerEpoch())) {
