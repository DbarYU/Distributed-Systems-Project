...
public class UDPMessageReceiver extends Thread implements LoggingServer {
    ...
    private static final Logger logger = Logger.getLogger(UDPMessageReceiver.class.getName() + Thread.currentThread().getName());
    public UDPMessageReceiver(LinkedBlockingQueue<Message> incomingMessages, InetSocketAddress myAddress, int myPort, PeerServer peerServer) throws IOException {
        ...
        String fileName = "logs/" + Globals.RUN_ID + "/edu.yu.cs.com3800.UDPMessageReceiver"
                + "-on-tcpPort" + tcpPort
                + "-Log.log";
        FileHandler fh;
        try {
